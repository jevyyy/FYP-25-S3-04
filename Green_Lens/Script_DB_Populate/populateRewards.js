...
export default function User_RewardPage() {
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      // 1️⃣ Fetch an unredeemed voucher
      const vouchersSnapshot = await getDocs(collection(rewardRef, 'vouchers'));
      const availableVoucherDoc = vouchersSnapshot.docs.find(doc => !doc.data().redeemed);

      if (!availableVoucherDoc) {
        Alert.alert('No available vouchers for this reward.');
        return;
      }

      const voucherData = availableVoucherDoc.data();

      // 2️⃣ Deduct points and update reward quantity
      await updateDoc(userRef, { totalPoints: increment(-selectedReward.cost) });
      await updateDoc(rewardRef, { quantity: increment(-1) });

      // 3️⃣ Mark voucher as redeemed
      const voucherRef = doc(rewardRef, 'vouchers', availableVoucherDoc.id);
      await updateDoc(voucherRef, { redeemed: true, redeemedBy: currentUser.uid, redeemedAt: new Date() });
      ...
      Alert.alert(`You redeemed ${selectedReward.title}! Your voucher: ${voucherData.code}`);
    } catch (err) {
      console.error(err);
