...
export default function Developer_SettingsPage() {
  ...
  const fetchImages = async (category) => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        const items = snapshot.docs.map((d) => ({ id: d.id, uri: d.data().imageUrl, name: d.data().name || d.data().uploadedBy || 'User Upload' }));
        setImages(items);
  ...
  // logout user and reset navigation
  const handleLogout = () => {
    Alert.alert('Logout', 'Are you sure you want to logout?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Logout', style: 'destructive', onPress: () => {
        navigation.dispatch(CommonActions.reset({ index: 0, routes: [{ name: 'GuestFlow', state: { routes: [{ name: 'Guest_HomePage' }] } }] }));
      }},
    ]);
  };
  ...
  // open external URL
  const openURL = (url) => {
    Linking.canOpenURL(url).then((supported) => (supported ? Linking.openURL(url) : Alert.alert("Cannot open URL"))).catch((err) => console.error("Error opening URL:", err));
  };

  // render collapsible dropdown option
  ...
  // select training category and fetch images
  const handleCategorySelect = (category) => { setSelectedCategory(category); fetchImages(category); };

  // confirm training and show buffer overlay
  const handleTrainConfirm = () => {
    setTrainModalVisible(false);
    setTrainingBuffer(true);
    setTimeout(() => { setTrainingBuffer(false); Alert.alert('Done', `Training Model Completed!`); setSelectedCategory(null); }, 5000);
  };

  // open pretrain modal
  const handlePretrainPress = () => setPretrainModalVisible(true);

  // select pretraining category and fetch images
  const handlePretrainCategorySelect = (category) => { setPretrainCategory(category); fetchPretrainImages(category); };

  // confirm pretraining and show buffer overlay
  const handlePretrainConfirm = () => {
    setPretrainModalVisible(false);
    setPretrainingBuffer(true);
    setTimeout(() => { setPretrainingBuffer(false); Alert.alert('Done', `Pre-Training Model Completed!`); setPretrainCategory(null); }, 5000);
  };

  // render single image card in modal
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }} showsVerticalScrollIndicator={false}>
        ...
        {/* dropdown sections */}
        {renderOption('Pre-train Model', showPretrain, () => setShowPretrain(!showPretrain),
          <TouchableOpacity style={styles.greenButton} onPress={handlePretrainPress}><Text style={styles.buttonText}>Pre-Train Model</Text></TouchableOpacity>
        )}
        {renderOption('Train Model', showTrain, () => setShowTrain(!showTrain),
          <TouchableOpacity style={styles.greenButton} onPress={handleTrainPress}><Text style={styles.buttonText}>Train Model</Text></TouchableOpacity>
        )}
        {renderOption('Deployment', showDeployment, () => setShowDeployment(!showDeployment),
          <TouchableOpacity style={styles.greenButton} onPress={() => openURL("https://drive.google.com/your-folder-link")}><Text style={styles.buttonText}>Deploy Model</Text></TouchableOpacity>
        )}
        {renderOption('Testing', showTesting, () => setShowTesting(!showTesting),
          <TouchableOpacity style={styles.greenButton} onPress={() => navigation.navigate('Developer_TestModelPage')}><Text style={styles.buttonText}>Test Model</Text></TouchableOpacity>
        )}
        <TouchableOpacity style={styles.option} onPress={() => navigation.navigate('FeedbackPage')}><Text style={styles.optionText}>Rate Us</Text></TouchableOpacity>
        {renderOption('App Info', showAppInfo, () => setShowAppInfo(!showAppInfo),
          <>
        ...
        <TouchableOpacity style={styles.option} onPress={handleLogout}><Text style={[styles.optionText, { color: 'red' }]}>Logout</Text></TouchableOpacity>
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            {!selectedCategory ? (
              ...
              <>
                <Text style={{ fontSize: 18, fontWeight: '600', marginBottom: 15 }}>{selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Dataset Images</Text>
                {loadingImages ? <ActivityIndicator size="large" color="#2E7D32" /> :
                  <FlatList data={images} keyExtractor={(item) => item.id} renderItem={renderImageItem} numColumns={2} contentContainerStyle={{ paddingBottom: 10 }} columnWrapperStyle={{ justifyContent: 'space-between', marginBottom: 10 }} />
                }
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 }}>
                  ...
                  <TouchableOpacity style={[styles.greenButton, { flex: 1, marginLeft: 5 }]} onPress={handleTrainConfirm}><Text style={styles.buttonText}>Train</Text></TouchableOpacity>
                </View>
              </>
            )}
          </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            {!pretrainCategory ? (
              ...
              <>
                <Text style={{ fontSize: 18, fontWeight: '600', marginBottom: 15 }}>{pretrainCategory.charAt(0).toUpperCase() + pretrainCategory.slice(1)} Dataset Images</Text>
                {loadingPretrainImages ? <ActivityIndicator size="large" color="#2E7D32" /> :
                  <FlatList data={pretrainImages} keyExtractor={(item) => item.id} renderItem={renderImageItem} numColumns={2} contentContainerStyle={{ paddingBottom: 10 }} columnWrapperStyle={{ justifyContent: 'space-between', marginBottom: 10 }} />
                }
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 }}>
                  ...
                  <TouchableOpacity style={[styles.greenButton, { flex: 1, marginLeft: 5 }]} onPress={handlePretrainConfirm}><Text style={styles.buttonText}>Pre-Train</Text></TouchableOpacity>
                </View>
              </>
            )}
          </View>
      ...
      {/* buffer overlay for training */}
      {trainingBuffer && <View style={styles.bufferOverlay}><View style={styles.bufferBox}><Text style={{ fontSize: 16 }}>Training model for {selectedCategory}...</Text><ActivityIndicator size="large" color="#2E7D32" style={{ marginTop: 10 }} /></View></View>}

      {/* buffer overlay for pretraining */}
      {pretrainingBuffer && <View style={styles.bufferOverlay}><View style={styles.bufferBox}><Text style={{ fontSize: 16 }}>Pre-training model for {pretrainCategory}...</Text><ActivityIndicator size="large" color="#2E7D32" style={{ marginTop: 10 }} /></View></View>}
    </View>
  );
}
