...
import { View, Text, Image, StyleSheet, TouchableOpacity, Alert, Platform, Linking, ActivityIndicator } from 'react-native';
...
export default function User_ViewSummaryPage({ route }) {
  ...
  const classifyImage = async () => {
    ...
    try {
      ...
      if (!healthData.modelLoaded) {
        throw new Error('AI model is not loaded on the server');
      }
  ...
  // Share function with SDK 54 compatibility
  const handleShare = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(shareUri, {
        dialogTitle: `Check out this ${objectName} I identified with Green Lens!`,
      });
    } catch (error) {
